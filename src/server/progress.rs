...
impl ProgressReporter {
    ...
    pub async fn get_operation_stats(&self, operation_id: &str) -> Option<OperationStats> {
        ...
        operations.get(operation_id).map(|op| {
            ...
            let current = op.latest_update.as_ref().map(|u| u.current).unwrap_or(0);
...
mod tests {
    ...
    fn test_progress_update_creation() {
        let update =
            ProgressUpdate::started("test-op".to_string(), "Starting test".to_string(), 100);
