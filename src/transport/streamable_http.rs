...
impl SessionManager {
    ...
    pub async fn store_event(&self, event: &McpEvent) -> McpResult<()> {
        self.event_store
            .store_event(&event.session_id, event.clone())
            .await
    }
    ...
    pub async fn subscribe_to_session(
        &self,
        _session_id: &str,
    ) -> impl futures_util::Stream<Item = McpEvent> {
                ...
                if !expired_sessions.is_empty() {
                    ...
                    for session_id in expired_sessions {
                        ...
                        if let Err(e) = event_store.remove_session(&session_id).await {
                            warn!(
                                "Failed to clean up event store for session {}: {}",
                                session_id, e
                            );
                        } else {
...
impl StreamableHttpTransport {
    ...
    pub async fn handle_post_request(
        ...
        if let Some(session_id) = session_id {
            ...
            let response = self
                .handle_initialization_request(&session_id, request_body)
                .await?;
    ...
    pub async fn handle_delete_request(&self, headers: &HeaderMap) -> McpResult<Response> {
        let session_id = headers.get("mcp-session-id").and_then(|h| h.to_str().ok());
    ...
    async fn handle_initialization_request(
        ...
        let request: JsonRpcRequest =
            serde_json::from_slice(&request_body).map_err(|e| McpError::Protocol {
    ...
    async fn handle_jsonrpc_request(
        ...
        let response = match request.method.as_str() {
            ...
            "tools/call" => {
                ...
                match tool_name {
                    "ratchet_list_available_tasks" => JsonRpcResponse {
                        jsonrpc: "2.0".to_string(),
                        id: request.id,
                        result: Some(serde_json::json!({
                            "content": [
                    ...
                    "ratchet_execute_task" => JsonRpcResponse {
                        jsonrpc: "2.0".to_string(),
                        id: request.id,
                        result: Some(serde_json::json!({
                            "content": [
